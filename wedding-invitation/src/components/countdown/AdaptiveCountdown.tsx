...
function UnitBox({ value, label }: UnitBoxProps) {
  return (
    <div className="flex flex-col items-center gap-2">
      <span className="font-icon text-dark text-5xl md:text-6xl leading-none tabular-nums">
        {String(value).padStart(2, "0")}
      </span>
      <span className="font-cinzel text-dark/60 text-xs tracking-widest uppercase">
        {label}
      </span>
    </div>
  );
  ...
  if (timeLeft.mode === "passed") {
    return (
      <div className="text-center">
        <p className="font-slight text-dark text-4xl">¡Feliz boda!</p>
      </div>
    );
  }

  const separator = (
    <span className="font-cinzel text-dark/40 text-3xl md:text-4xl self-start pt-3">:</span>
  );

  return (
    <div className="flex flex-col items-center gap-4">
      <p className="font-cinzel text-dark/50 tracking-[0.4em] uppercase text-xs">
        Cuenta regresiva
      </p>
